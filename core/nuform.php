...
function nuGetFormObject($F, $R, $OBJS, $tabs = null){
	...
	if(!isset($f->table) || $f->table == ''){
		...
		$s	= "Select * FROM " . nuIdentCol($f->table) . " WHERE " . nuIdentCol($f->primary_key) . " = ?";
...
function nuGetSubformRecords($R, $A){
	...
	$s = "SELECT $f->primary_key $f->from WHERE ($R->sob_subform_foreign_key = '$R->subform_fk') $w $f->order"; // xx
...
function nuBuildTabList($i){
	...
	$s = "
		...
		GROUP BY zzzzsys_tab_id, syt_zzzzsys_form_id, syt_title, syt_order, syt_help
