			...
			if(nuEditedRow($edited[$r])){
				...
				if($action == 'save'){
					...
					if($pv == '-1'){
						...
						$sql		= "UPDATE $table SET $fs WHERE " . nuIdentCol($pk) . " = '$pv';";

						$S[]		= $sql;

						if($log){

							$sql	= "SELECT $table" . "_nulog FROM $table WHERE " . nuIdentCol($pk) . " '$pv';";
							$logt	= nuRunQuery($sql);
		...
		for($i = 0 ; $i < count($deleted) ; $i++){
			...
			if($action == 'delete' || $deleted[$i] == '1'){
				...
				if($id != '-1'){
					...
					$sql	= "DELETE FROM $table WHERE " . nuIdentCol($pk) . " = '$id';";
					$S[]	= $sql;
...
function nuDeleteRow($r, $p){
	...
	nuRunQuery("DELETE FROM " . nuIdentCol($r->sfo_table) . " WHERE " . nuIdentCol($r->sfo_primary_key) . " = ? ", array($p));

}

function nuInsertRow($r, $p){

	$T	= nuRunQuery("SELECT COUNT(*) FROM " . nuIdentCol($r->sfo_table) . " WHERE " . nuIdentCol($r->sfo_primary_key) . " = ? ", array($p));
	$R	= db_fetch_row($T);
	
	if($R[0] == 0){
		nuRunQuery("INSERT INTO " . nuIdentCol($r->sfo_table) . " (" . nuIdentCol($r->sfo_primary_key) . ") VALUES (?) ", array($p));
	}
...
function nuUpdateRow($r, $p, $row, $FK){
	...
	if(count($set) > 0){
		...
		$q[]		= $p;
		$s	= "UPDATE " . nuIdentCol($r->sfo_table) . " SET " . implode(', ', $set) . " WHERE " . nuIdentCol($r->sfo_primary_key) . " = ? ";
		nuRunQuery($s, $q);
...
function nuCloneForm(){
	...
	$s		= nuCreateTableFromSelectSQL("p$TT", "SELECT * FROM zzzzsys_php WHERE zzzzsys_php_id LIKE CONCAT(?,'%')");
	$t		= nuRunQuery($s, array($f));

	$s		= nuCreateTableFromSelectSQL("o$TT", "SELECT * FROM zzzzsys_object WHERE sob_all_zzzzsys_form_id = ?");
	$t		= nuRunQuery($s, array($f));

	$s		= nuCreateTableFromSelectSQL("e$TT", "SELECT * FROM zzzzsys_event WHERE sev_zzzzsys_object_id IN(SELECT zzzzsys_object_id FROM o$TT)");
